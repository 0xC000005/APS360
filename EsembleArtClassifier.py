...
if __name__ == '__main__':
    ...
    efficientnet = efficientnet_v2_s(weights='DEFAULT').to(device)
    ...
    vit.head = nn.Linear(768, num_classes).to(device).to(device)
    efficientnet.classifier[1] = nn.Linear(1280, num_classes).to(device)
    ...
    model1_name = 'VitArtTorchClassifierWithWeight20241125025107.pth'
    model2_name = 'efficientnet_ArtClassifier20241124081624.pth'
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # only keep the first 11 probabilities
            prob1 = prob1[:, :11]
            print("vit prob: ", prob1)
            prob2 = torch.nn.functional.softmax(outputs2, dim=1)
            print("efficientnet prob: ", prob2)
