...
if __name__ == '__main__':
    ...
    efficientnet = efficientnet_v2_s(weights=None, num_classes=num_classes).to(device)
    ...
    train_dataset = ds['train'].map(train_transform_func, num_proc=16, cache_file_name="~/.cache/huggingface/cached_train_transform.arrow").with_format('torch')
    val_dataset = ds['validation'].map(val_transform_func, num_proc=16, cache_file_name="~/.cache/huggingface/cached_val_transform.arrow").with_format('torch')
    test_dataset = ds['test'].map(val_transform_func, num_proc=16, cache_file_name="~/.cache/huggingface/cached_test_transform.arrow").with_format('torch')
    ...
    # save the model
    model_name = 'efficientnet_ArtClassifier' + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + '.pth'
    ...
    plt.grid(True)
    # Save the plot
    plt.savefig('efficientnet_losses' + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + '.png')
    plt.close()
    ...
    plt.grid(True)
    # Save the plot
    plt.savefig('efficientnet_accuracy' + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + '.png')
    plt.close()
